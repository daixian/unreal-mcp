...
| `create_data_asset` | `data_asset_name`、`data_asset_class`、可选 `path="/Game"`、`unique_name=False` | 创建 `DataAsset` 子类实例，并通过 `DataAssetFactory` 写入目标类。（本地Python） |
| `create_primary_data_asset` | `data_asset_name`、`data_asset_class`、可选 `path="/Game"`、`unique_name=False` | 创建 `PrimaryDataAsset` 子类实例；会强校验目标类确实继承自 `PrimaryDataAsset`。（本地Python） |
| `create_curve` | `curve_name`、可选 `path="/Game"`、`curve_type="CurveFloat"`、`unique_name=False` | 创建 `CurveFloat`、`CurveVector` 或 `CurveLinearColor` 资产。（本地Python） |
| `create_data_table` | `table_name`、`row_struct`、可选 `path="/Game"`、`unique_name=False` | 创建 `DataTable` 资产，并把指定原生或用户自定义结构体写入工厂 `Struct` 属性。（本地Python） |
| `get_data_table_rows` | 任一 `DataTable` 资产定位字段、可选 `row_name` | 读取 `DataTable` 的全部行，或只读取指定行；返回导出后的 JSON 行对象数组。（本地Python） |
| `import_data_table` | 任一 `DataTable` 资产定位字段、`source_file`、可选 `format="auto"`、`save_asset=True` | 从 CSV 或 JSON 文件整表导入 `DataTable` 内容。（本地Python） |
| `set_data_table_row` | 任一 `DataTable` 资产定位字段、`row_name`、`row_data={}`、可选 `save_asset=True` | 以导出 JSON 的字段名为准，新增或覆盖单行数据，再整表回填到 `DataTable`。（本地Python） |
| `export_data_table` | 任一 `DataTable` 资产定位字段、`export_file`、可选 `format="auto"` | 把 `DataTable` 当前内容导出为 CSV 或 JSON 文件。（本地Python） |
| `remove_data_table_row` | 任一 `DataTable` 资产定位字段、`row_name`、可选 `save_asset=True` | 删除 `DataTable` 中的指定行，并返回剩余行信息。（本地Python） |
| `save_asset` | 任一资产定位字段、`only_if_dirty=False` | 保存指定资产；可选择只在 Dirty 时保存。 |
| `checkout_asset` | 任一资产定位字段 | 通过源码控制系统签出指定资产；当前为本地Python 主实现。 |
| `submit_asset` | 任一资产定位字段、`description`、可选 `save_asset=True`、`silent=False`、`keep_checked_out=False` | 通过源码控制系统提交指定资产；当前为本地Python 主实现。 |
| `revert_asset` | 任一资产定位字段、可选 `silent=False` | 通过源码控制系统还原指定资产；当前为本地Python 主实现。 |
| `get_source_control_status` | 任一资产定位字段 | 查询指定资产当前的源码控制状态；当前为本地Python 主实现。 |
...
| `add_material_expression` | 任一基础材质定位字段、`expression_class`、可选 `node_position=[X,Y]`、`selected_asset_path`、`property_values={}` | 在材质图里创建一个表达式节点，并可附带基础属性覆写。（本地Python） |
| `connect_material_expressions` | 任一基础材质定位字段、`from_expression_*`、可选 `from_output_name`、以及 `to_expression_*` 或 `material_property` 二选一 | 连接两个表达式，或把表达式输出接到材质属性。（本地Python） |
| `layout_material_graph` | 任一基础材质定位字段 | 自动整理材质图里全部表达式节点布局。（本地Python） |
| `compile_material` | 任一基础材质定位字段 | 触发材质重新编译并保存。（本地Python） |
...
成功结果在 `create_asset` 的基础字段上，额外包含：
...
- 这两条命令都统一复用 `DataAssetFactory`，而不是绕回通用 `create_asset` 让调用侧自己猜工厂。
- `data_asset_class` 支持 UE Python 短类名，也支持完整类路径；既可以传原生类，也可以传继承自 `DataAsset` / `PrimaryDataAsset` 的蓝图生成类路径。
- `DataAsset` 和 `PrimaryDataAsset` 自身都是抽象基类，不能直接实例化；调用时必须传具体子类，例如项目自定义数据资产类，或内置的 `PrimaryAssetLabel` 这类可实例化类型。
- `create_data_asset` 只要求目标类继承自 `DataAsset`；`create_primary_data_asset` 会进一步强校验目标类必须继承自 `PrimaryDataAsset`，避免创建出“名字像 Primary，实际不是”的错误资产。
- 返回里的 `data_asset_class` 会同时给出类名和类路径，便于自动化回读确认最终实例类型。
...
- `curve_type` 当前支持 `CurveFloat`、`CurveVector`、`CurveLinearColor`，也接受简写别名 `float`、`vector`、`linear_color`。
- Unreal 侧会自动映射到对应工厂：`CurveFloatFactory`、`CurveVectorFactory`、`CurveLinearColorFactory`。
- 这条命令本质上仍是标准资产工厂创建，因此保持为“薄 C++ 外壳 + 本地Python 主实现”。
...
- `row_struct` 支持 UE Python 原生结构体短名，例如 `Vector`、`Transform`，也支持用户自定义结构体对象路径，例如 `/Game/Data/ST_ItemRow.ST_ItemRow`。
- Unreal 侧会使用 `DataTableFactory`，并把解析后的结构体对象写入工厂 `Struct` 属性，再走标准资产创建链路。
- 当前不会在本地 Python 里额外伪造行结构约束；如果传入的结构体不符合 DataTable 要求，会由 UE 工厂阶段直接返回错误。

### `get_data_table_rows` / `import_data_table` / `set_data_table_row` / `export_data_table` / `remove_data_table_row`
...
- `get_data_table_rows`
  - 额外返回 `returned_row_count` 与 `rows`
  - 传 `row_name` 时会只返回目标行，不存在则直接报错
- `import_data_table`
  - 额外返回 `source_file`、`format`
  - `format="auto"` 时会按文件扩展名自动推断 `.csv` / `.json`
  - 导入完成后会回读 `row_count` 与 `row_names`
- `set_data_table_row`
  - 额外返回 `row_name`、`row_data`、`replaced_existing`
  - `row_data` 必须是对象，字段名应与 `DataTable.export_to_json_string()` 导出的字段名一致
  - Unreal 侧会先导出整表 JSON，按 `row_name` 合并单行后，再通过 `fill_from_json_string(...)` 回填整表
- `export_data_table`
  - 额外返回 `export_file`、`format`
  - `format="auto"` 时会按导出文件扩展名自动推断 `.csv` / `.json`
- `remove_data_table_row`
  - 额外返回 `row_name` 与 `removed=true`
  - 当前直接复用 UE5.7 Python 已公开的 `DataTableFunctionLibrary.remove_data_table_row(...)`

说明：

- 这 3 条命令都要求目标资产真实是 `DataTable`，不会接受普通资产路径。
- `import_data_table` / `export_data_table` 都走文件路径，不额外在 MCP 回包里内嵌整段 CSV / JSON 文本，避免大表时把 TCP 返回撑爆。
- `import_data_table` 当前属于“整表覆盖式导入”。
  - 也就是说，导入文件会覆盖当前 `DataTable` 的全部内容，不做按行 merge。
- `set_data_table_row` 会强制把传入行的 `Name` 字段覆盖成 `row_name`，避免外部传参里出现“行名和 JSON 体不一致”的脏数据。
- 由于 UE5.7 Python 已稳定暴露 `export_to_json_string(...)`、`fill_from_json_string(...)`、`get_row_names()` 与 `remove_data_table_row(...)`，因此这条链路继续保持为“薄 C++ 外壳 + 本地Python 主实现”。
...
- 当前实现层已经统一收敛到 `Plugins/UnrealMCP/Content/Python/commands/assets/asset_commands.py` 的本地 Python 实现。
...
- 当前已接入“薄 C++ 外壳 + 本地 Python 实现”，内部调用 `EditorAssetLibrary.checkout_asset(...)`。
- 该工具依赖 UE 编辑器当前已经配置可用的 Source Control Provider；若项目未连接源码控制，UE 会返回失败。
- 成功响应会带 `implementation=local_python`。
...
- 当前已接入“薄 C++ 外壳 + 本地 Python 实现”，内部调用 `SourceControl.check_in_file(...)`。
- `description` 为必填提交说明。
- 默认会先保存已加载资产，再执行提交；如不希望自动保存，可传 `save_asset=false`。
- 若当前编辑器未启用或未连接源码控制 Provider，会直接返回失败，并在 `error` / `last_error` 中给出原因。
- 成功响应会带 `implementation=local_python`。
...
- 当前已接入“薄 C++ 外壳 + 本地 Python 实现”，内部调用 `SourceControl.revert_file(...)`。
- 该工具用于丢弃源码控制中的本地改动；若 Provider 未启用或不可用，会直接返回失败。
- 成功响应会带 `implementation=local_python`。
...
- `checked_out_other`
- `is_checked_out_in_other_branch`
- `is_modified_in_other_branch`
- `previous_user`

说明：

- 当前已接入“薄 C++ 外壳 + 本地 Python 实现”，内部调用 `SourceControl.query_file_state(...)`。
- 即使当前 Provider 未启用，也会尽量返回当前 Provider 可用性与状态结构，方便自动化判断。
- 成功响应会带 `implementation=local_python`。
...
```json
{
  "tool": "get_source_control_status",
  "args": {
    "asset_path": "/Game/CodexAutomation/BP_CodexVarDefault_20260404_0735"
  }
}
```
...
```json
{
  ...
  "args": {
    "asset_path": "/Game/CodexAutomation/BP_CodexVarDefault_20260404_0735",
    "description": "Codex automation asset update",
    "save_asset": true,
...
```json
{
  "tool": "export_data_table",
  "args": {
    "asset_path": "/Game/CodexAutomation/DT_CodexSample",
    "export_file": "X:/work/conan2/veidon/UE5/XREmpty/Saved/CodexAutomation/DT_CodexSample.json",
    "format": "json"
  }
}
```
...
```json
{
  "tool": "import_data_table",
  "args": {
    "asset_path": "/Game/CodexAutomation/DT_CodexSample",
    "source_file": "X:/work/conan2/veidon/UE5/XREmpty/Saved/CodexAutomation/DT_CodexSample.csv",
    "format": "csv",
    "save_asset": true
  }
}
