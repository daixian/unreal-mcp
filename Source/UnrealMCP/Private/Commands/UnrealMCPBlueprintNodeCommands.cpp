...
namespace
{
/** @brief 自动生成节点的注释标记，用于后续清理。 */
const TCHAR* ZSpaceAutoGenNodeComment = TEXT("MCP_ZSPACE_MINIMAL_AUTOGEN");

/**
 * @brief 在目标类中按候选名称查找函数。
 * @param [in] TargetClass 目标类。
 * @param [in] Candidates 候选函数名列表。
 * @return UFunction* 找到的函数，未找到返回 nullptr。
 */
...
/**
 * @brief 获取节点中第一个数据引脚（排除执行引脚）。
 * @param [in] Node 目标节点。
 * @param [in] Direction 引脚方向过滤。
 * @return UEdGraphPin* 数据引脚，未找到返回 nullptr。
 */
...
/**
 * @brief 连接两枚引脚。
 * @param [in] SourcePin 源引脚。
 * @param [in] TargetPin 目标引脚。
 ...
 * @param [in] SourcePinCandidates 源引脚候选名。
 * @param [in] TargetNode 目标节点。
 * @param [in] TargetPinCandidates 目标引脚候选名。
 * @return bool 连接成功返回 true。
 */
 ...
 * @brief 设置节点引脚默认值。
 * @param [in] Node 目标节点。
 * @param [in] PinCandidates 引脚候选名。
 * @param [in] Value 要设置的默认值文本。
 */
...
/**
 * @brief 确保 Blueprint 中存在指定变量，不存在则创建。
 * @param [in] Blueprint 目标 Blueprint。
 * @param [in] VariableName 变量名称。
 * @param [in] VariableType 变量类型。
 * @return bool 已存在或创建成功返回 true。
 */
...
/**
 * @brief 自动构建 ZSpace 最小交互节点流程。
 * @param [in] Params 配置参数。
 * @return TSharedPtr<FJsonObject> 构建结果。
 */
